...
class BinaryTree:
    ...
    The data type of the parameter value is  the value of the root.
    '''
    ...
    '''
    This method returns the result of the lambda expression
    '''
    def filter(self, L, node):
        return filter(L, self.getTreeAsArray(node))
    
    
    '''
    This method converts the tree to an array
    insert and extend are methods of the Python
    the  method 'insert' adds an item to the end of the list
    the method 'extend' extends the list by appending all the items in the given list
    '''
    def getTreeAsArray(self, node):
        result = []
        if node :
            result.insert(len(result), node.getData())
            result.extend(self.getTreeAsArray(node.getLeft()))
            result.extend(self.getTreeAsArray(node.getRight()))
        return result
